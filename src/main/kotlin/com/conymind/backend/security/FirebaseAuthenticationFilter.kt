...
class FirebaseAuthenticationFilter(private val debugModeToken : String?) : OncePerRequestFilter() {

    @Throws(IOException::class, ServletException::class)
    override fun doFilterInternal(request: HttpServletRequest, response: HttpServletResponse, chain: FilterChain) {
        val token = request.getHeader("Authorization")
        if (token != null && token.startsWith("Bearer ")) {
            if (debugModeToken != null && token.substring(7) == debugModeToken) {
                val userDetails = FirebaseUserDetails("tester@conymind.com", "tester")
